...
resource "aws_cloudwatch_metric_alarm" "master_cpu_utilization" {
    count           = ("${data.aws_elasticsearch_domain.target.cluster_config.dedicated_master_count}" == 0) ? 0 : 1
...
resource "aws_cloudwatch_metric_alarm" "master_reachable_from_node" {
  count             = ("${data.aws_elasticsearch_domain.target.cluster_config.dedicated_master_enabled}" == 0)? 0 : 1
